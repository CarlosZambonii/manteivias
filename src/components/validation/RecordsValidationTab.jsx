...
const RecordsValidationTab = ({ worksiteFilter }) => {
  ...
  // Fetch Worksites based on role and worksiteFilter
  ...
  const fetchRecordsData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      // Apply Worksite Filter based on role and selection
  ...
  }, [filterMonthTime, filterWorksiteId, filterStatus, worksites, worksiteFilter, toast, user]);

  // Initial Worksite Fetch
  useEffect(() => {
    if (user?.id) {
        fetchWorksites();
    }
  ...
  // Record Fetch on filter change or worksites load
  useEffect(() => {
    fetchRecordsData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filterMonthTime, filterWorksiteId, filterStatus, worksiteFilter, worksites.length, user?.tipo_usuario, user?.categoria]);
  ...
  const handleExportXLSX = () => {
    ...
    const filename = `registos_validacao_${format(new Date(), 'yyyy-MM-dd')}.xlsx`;
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-center p-8 bg-muted/20 rounded-lg border-2 border-dashed">
          ...
          <Button variant="link" onClick={() => setFilters(prev => ({ ...prev, status: 'Todos', search: '', shift: 'all' }))}>
             Limpar Filtros
          </Button>
        </div>
      ) : (
