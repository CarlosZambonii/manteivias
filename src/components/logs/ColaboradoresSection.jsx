...
export default function ColaboradoresSection({ colaboradoresData = [], loading, search = '' }) {
  const [mode, setMode] = useState("top");
  const [metric, setMetric] = useState("acessos");
  const [limit, setLimit] = useState(999);
  const [categoria, setCategoria] = useState("Todas as Categorias");

  const filtered = colaboradoresData
    .filter(c => categoria === "Todas as Categorias" || c.categoria === categoria)
  ...
  return (
    <div className="space-y-3">
      ...
      {loading ? (
        ...
        <div className="bg-card rounded-xl border border-border/60 py-8 text-center text-sm text-muted-foreground">
          Sem dados no período.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {items.map((col, i) => {
            const badge = atividadeBadge[col.atividade] || atividadeBadge.baixa;
            return (
              <div
                key={i}
                className={`bg-card rounded-xl border p-4 hover:border-primary/20 transition-colors ${
                  i === 0 ? isTop ? "border-emerald-500/30" : "border-red-500/30" : "border-border/60"
                }`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
