...
import FolhaFiscalTemplate from "./FolhaFiscalTemplate";

export default function FolhaFiscalPreview({ data, showAv, onClose }) {
    ...
    <div className="fixed inset-0 bg-black/70 flex flex-col print:bg-white print:relative print:inset-auto" style={{ zIndex: 9999 }}>
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-gray-900 border-b border-gray-700 shrink-0 print:hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" variant="ghost" className="text-white hover:bg-gray-700 ml-2" onClick={() => window.print()}>
      ...
      <div className="flex-1 overflow-auto bg-gray-200 p-4 print:p-0 print:overflow-visible print:bg-white">
        ...
        >
          <div className="bg-white shadow-2xl print:shadow-none" style={{ margin: "0 auto", padding: "16px 14px", maxWidth: 1100 }}>
            <FolhaFiscalTemplate data={data} showAv={showAv} />
          </div>
        </div>
      </div>
    </div>,
