...
import { utilizacaoData } from "@/lib/logsData";
...
export default function UtilizacaoSection() {
  ...
  const maxVal = Math.max(...utilizacaoData.map(x => x[metric]));
    ...
    <div className="bg-card rounded-xl border border-border/60 overflow-hidden">
      <div className="px-5 py-3.5 border-b border-border/60 flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Métricas de utilização de cada página</p>
        </div>
        <RankingFilter
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border/40 hover:bg-transparent">
              ...
              <TableHead className="text-xs font-semibold text-muted-foreground">Página</TableHead>
